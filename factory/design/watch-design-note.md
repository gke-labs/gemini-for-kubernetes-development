...
* **Trigger**: Opted in via the `overseer/review` label (or `<triggerLabel>/review`) on the PR itself (`shouldAutoReviewPR`), CI checks are passing (`!hasFailure && !isConflicting`), PR is not yet approved (`!isApproved`), no new unaddressed comments exist (`!hasNewComments`), and the current `HEAD SHA` has not yet been reviewed by the Reviewer Bot identity (`state.lastReviewedSHA != headSHA`).
* **Opting In / Out (Non-Sticky Label)**: A PR inherits the review label from its referenced parent Issue along with the Issue's other labels (`syncReferencedIssueLabels`), so labelling the Issue is enough to opt its PRs in. The review label is registered as **non-sticky** (`nonStickyLabels`), meaning it is handed over once instead of being reconciled every cycle: `filterReclaimedLabels` consults the PR's `unlabeled` events and never re-applies a non-sticky label the PR has already had removed. Taking the label off a PR is therefore how a human takes over its review, without opting the parent Issue's other PRs out. The review opt-in is the only non-sticky label today; every other inherited label still follows the parent Issue and comes back if removed.
