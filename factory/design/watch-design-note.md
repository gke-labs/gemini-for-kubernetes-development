...
* **Giving Up State & Reset Behavior**:
  ...
  * **Human Feedback / Comments**: Posting new instructions or comments on the PR does *not* reset the giving up state. Furthermore, if a PR has failing CI checks while in the given-up state, Overseer currently skips further evaluation of the PR, blocking comment handling (`pr-comments`) until a new commit is pushed.
