...
function runAgent {
    ...
    # Check for thread resumption in workflow mode
    RESUME_FLAG=""
    if [ "$AGENT_MODE" = "workflow" ]; then
        ...
        if [ -n "$LAST_SESSION" ]; then
            SESSION_ID=$(basename "$LAST_SESSION" .json | sed 's/session-//')
            echo "Found previous session: ${SESSION_ID}. Resuming thread..."
            RESUME_FLAG="--resume ${SESSION_ID}"
        else
            echo "No previous session found. Starting new thread..."
        fi
    fi
