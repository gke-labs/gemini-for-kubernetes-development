...
func runPR(ctx context.Context, number int, taskType string, submit bool, customPrompt string, bypassRetryLimit bool) error {
	...
	if err == nil {
		...
		if taskType == "investigate-failures" {
			...
			if botLogin != "" {
				lastHumanCommitTime, err := github.GetLastHumanCommitTime(ctx, ghClient.Client, owner, repo, number, botLogin, pr.GetCreatedAt())
				if err != nil {
					...
					if !bypassRetryLimit && investigateFailuresCount >= 2 {
						...
						if !hasComment {
							msg := github.LimitReachedCommentBody
