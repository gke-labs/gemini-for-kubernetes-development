...
func runPR(ctx context.Context, number int, taskType string, submit bool, customPrompt string) error {
	...
	if err == nil {
		...
		if taskType == "investigate-failures" {
			...
			if botLogin != "" {
				lastHumanCommitTime, err := github.GetLastHumanCommitTime(ctx, ghClient.Client, owner, repo, number, botLogin)
				if err != nil {
					...
					reportCount, err := github.CountInvestigationReportsSince(ctx, ghClient.Client, owner, repo, number, lastHumanCommitTime)
					if err != nil {
						fmt.Printf("Warning: failed to count investigation reports in comments: %v\n", err)
					...
					if investigateFailuresCount >= 2 {
						...
						hasComment, err := github.HasLimitReachedComment(ctx, ghClient.Client, owner, repo, number, lastHumanCommitTime)
						if err != nil {
							fmt.Printf("Warning: failed to check for limit-reached comment: %v\n", err)
						} else if !hasComment {
							...
							_, _, err = ghClient.Issues.CreateComment(ctx, owner, repo, number, &githubv39.IssueComment{Body: &msg})
							if err != nil {
								fmt.Printf("Warning: failed to post limit-reached comment: %v\n", err)
							}
						}
						return nil
					}
