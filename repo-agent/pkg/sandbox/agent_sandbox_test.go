...
func TestSkipDevcPrefix(t *testing.T) {
	tests := []struct {
		name           string
		inputName      string
		skipDevcPrefix bool
		...
		{
			name:           "WithPrefix_Default",
			inputName:      "test",
			skipDevcPrefix: false,
			expectedName:   "devc-test",
		},
		{
			name:           "WithPrefix_ExplicitSkip",
			inputName:      "test",
			skipDevcPrefix: true,
			expectedName:   "test",
		},
		{
			name:           "AlreadyPrefixed_Default",
			inputName:      "devc-test",
			skipDevcPrefix: false,
			expectedName:   "devc-test",
		},
		{
			name:           "AlreadyPrefixed_ExplicitSkip",
			inputName:      "devc-test",
			skipDevcPrefix: true,
			expectedName:   "devc-test",
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			opt := AgentSandboxOptions{
				...
				SkipDevcPrefix: tt.skipDevcPrefix,
			}
			sandbox, _ := NewAgentSandbox(opt)

			metadata := sandbox.Object["metadata"].(map[string]interface{})
			name := metadata["name"].(string)
			if name != tt.expectedName {
				t.Errorf("expected metadata.name %s, got %s", tt.expectedName, name)
			}

			podTemplate := sandbox.Object["spec"].(map[string]interface{})["podTemplate"].(map[string]interface{})
			podSpec := podTemplate["spec"].(map[string]interface{})
			containers := podSpec["containers"].([]interface{})
			container := containers[0].(map[string]interface{})
			env := container["env"].([]interface{})
			...
			for _, e := range env {
				...
				if envVar["name"] == "NAME" {
					found = true
					if envVar["value"] != tt.expectedName {
						t.Errorf("expected NAME env %s, got %v", tt.expectedName, envVar["value"])
					}
					break
				}
			}
			if !found {
				t.Errorf("NAME env var not found")
			}
		})
	}
}
