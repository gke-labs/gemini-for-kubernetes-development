...
function checkoutPRBranch {
    ...
    PREV_COMMIT=$(cd "/workspaces/${REPO_NAME}" && git rev-parse HEAD 2>/dev/null || true)
    export PREV_COMMIT
    echo "Previous PR commit: ${PREV_COMMIT}"
}
...
function postFeedbackDiffComment {
    echo "Running postFeedbackDiffComment..."
    pushd "/workspaces/${REPO_NAME}" > /dev/null
    
    NEW_COMMIT=$(git rev-parse HEAD 2>/dev/null || true)

    if [ -n "$PREV_COMMIT" ] && [ -n "$NEW_COMMIT" ] && [ "$PREV_COMMIT" != "$NEW_COMMIT" ]; then
        echo "HEAD changed from ${PREV_COMMIT} to ${NEW_COMMIT}."

        REPO_FULL_NAME=""
        if command -v gh >/dev/null 2>&1; then
            REPO_FULL_NAME=$(gh repo view --json nameWithOwner --jq .nameWithOwner 2>/dev/null || true)
        fi
        if [ -z "$REPO_FULL_NAME" ] && [ -n "$CLONE_URL" ]; then
            REPO_FULL_NAME=$(echo "$CLONE_URL" | sed -E 's|https://github.com/([^/]+/[^/.]+)(\.git)?|\1|')
        fi
        if [ -z "$REPO_FULL_NAME" ] && [ -n "$REPO_NAME" ]; then
            REPO_FULL_NAME="$REPO_NAME"
        fi

        DIFF_URL="https://github.com/${REPO_FULL_NAME}/compare/${PREV_COMMIT}...${NEW_COMMIT}"
        echo "Diff URL: ${DIFF_URL}"

        if [ -n "$PR_NUMBER" ] && [ "$PR_NUMBER" != "null" ]; then
            LABEL_FOOTER="${TRIGGER_LABEL_VAL:-${TRIGGER_LABEL:-Overseer}}"
            
            ALREADY_POSTED=false
            if command -v gh >/dev/null 2>&1; then
                if gh issue view "${PR_NUMBER}" --json comments --jq '.comments[].body' 2>/dev/null | grep -q "${PREV_COMMIT:0:7}...${NEW_COMMIT:0:7}" || gh issue view "${PR_NUMBER}" --json comments --jq '.comments[].body' 2>/dev/null | grep -q "${DIFF_URL}"; then
                    ALREADY_POSTED=true
                fi
            fi

            if [ "$ALREADY_POSTED" = false ]; then
                COMMENT_BODY="Force pushed new commit after addressing feedback. Diff between previous commit (${PREV_COMMIT:0:7}) and new commit (${NEW_COMMIT:0:7}): ${DIFF_URL}\n\n*(This comment was generated by ${LABEL_FOOTER})*"
                echo "Posting diff comment to PR #${PR_NUMBER}..."
                gh issue comment "${PR_NUMBER}" --body "$(echo -e "$COMMENT_BODY")" || gh pr comment "${PR_NUMBER}" --body "$(echo -e "$COMMENT_BODY")" || true
            else
                echo "Diff comment already posted on PR #${PR_NUMBER}."
            fi
        fi
    else
        echo "No commit changes detected after addressing feedback."
    fi
    popd > /dev/null
}
