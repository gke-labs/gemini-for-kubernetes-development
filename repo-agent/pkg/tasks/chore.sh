...
Only output the commit message itself.")
        ...
        cat <<OVERSEER_EOF > /tmp/pr_body.txt
...
OVERSEER_EOF
        ...
        PR_URL=$(gh pr create --title "chore: ${CHORE_NAME}" --body-file /tmp/pr_body.txt --head "${FORK_OWNER}:${BRANCH_NAME}" --base "${BASE_BRANCH}" || true)
