...
func (r *Reconciler) reconcilePRFailures(ctx context.Context, repoWatch *reviewv1alpha1.RepoWatch, sandbox *unstructured.Unstructured, issue *github.Issue, ghClient *github.Client) error {
	...
	if failed {
		...
		if botLogin != "" {
			lastHumanCommitTime, err := pkg_github.GetLastHumanCommitTime(ctx, ghClient, owner, repo, *pr.Number, botLogin, pr.GetCreatedAt())
			if err != nil {
			...
			reportCount, hasComment, err := pkg_github.GetInvestigationStatsSince(ctx, ghClient, owner, repo, *pr.Number, lastHumanCommitTime, botLogin)
			if err != nil {
			...
			if investigateFailuresCount >= 2 {
				...
				if !hasComment {
					msg := pkg_github.LimitReachedCommentBody
