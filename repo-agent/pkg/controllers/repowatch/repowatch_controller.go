...
func (r *Reconciler) reconcilePRFailures(ctx context.Context, repoWatch *reviewv1alpha1.RepoWatch, sandbox *unstructured.Unstructured, issue *github.Issue, ghClient *github.Client) error {
	...
	if failed {
		...
		if botLogin != "" {
			lastHumanCommitTime, err := pkg_github.GetLastHumanCommitTime(ctx, ghClient, owner, repo, *pr.Number, botLogin)
			if err != nil {
				log.Error(err, "unable to get last human commit time", "pr", *pr.Number)
			} else if !lastHumanCommitTime.IsZero() {
				...
				reportCount, err := pkg_github.CountInvestigationReportsSince(ctx, ghClient, owner, repo, *pr.Number, lastHumanCommitTime)
				if err != nil {
					log.Error(err, "unable to count investigation reports in comments", "pr", *pr.Number)
				} else if reportCount > investigateFailuresCount {
				...
				if investigateFailuresCount >= 2 {
					...
					// Check if we already commented about this since the last human commit
					hasComment, err := pkg_github.HasLimitReachedComment(ctx, ghClient, owner, repo, *pr.Number, lastHumanCommitTime)
					if err != nil {
						log.Error(err, "unable to check for limit-reached comment", "pr", *pr.Number)
					} else if !hasComment {
						msg := "The bot has reached its retry limit (2) for investigating failures on this PR since the last human commit. Human intervention is required to address sticky failures.\n\n*(This comment was generated by Overseer)*"
						_, _, err = ghClient.Issues.CreateComment(ctx, owner, repo, *pr.Number, &github.IssueComment{Body: &msg})
						if err != nil {
