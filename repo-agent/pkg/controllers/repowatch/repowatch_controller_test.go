...
// TestReconciler_Reconcile_InvalidEmailInSecret verifies that the reconciler correctly handles
// an invalid email in the secret by falling back to a noreply email address.
func TestReconciler_Reconcile_InvalidEmailInSecret(t *testing.T) {
	g := gomega.NewWithT(t)
	...
	// 2. Initialize the fake client
	fakeClient := clientfake.NewClientBuilder().WithScheme(s).WithStatusSubresource(&reviewv1alpha1.RepoWatch{}).Build()

	// 3. Create mock GitHub client
	// Mock response for user endpoint with empty email (common for privacy enabled users)
	// ID: 12345, Login: test-user
	userJSON := `{"login": "test-user", "id": 12345, "name": "Test User", "email": ""}`

	// Define the invalid email string from the issue
	invalidEmail := "codebot-robot @repo-agent/k8s/crds/syncer.gemini.google.com_syncers.yaml"
	...
	// 4. Create Reconciler with a factory that returns the invalid email in config
	r := &Reconciler{
		Client: fakeClient,
		Scheme: s,
		NewGithubClient: func(_ context.Context, _ client.Client, _ *reviewv1alpha1.RepoWatch) (*github.Client, map[string]string, error) {
			return ghClient, map[string]string{
				"pat":   "test-pat",
				"email": invalidEmail,
			}, nil
		},
	...
	// 5. Create RepoWatch and Secret
	objName := "test-repowatch-invalid-email"
	objNamespace := "default"
	req := reconcile.Request{
		NamespacedName: types.NamespacedName{Name: objName, Namespace: objNamespace},
	}
	...
	secret := &corev1.Secret{
		...
		Data: map[string][]byte{
			"pat":   []byte("test-pat"),
			"email": []byte(invalidEmail),
		},
	}
	g.Expect(fakeClient.Create(context.Background(), secret)).To(gomega.Succeed())

	// 6. Call Reconcile
	// Mock HTTP to return an Issue
	mockHTTPClient.Transport = &mockRoundTripper{
		responses: map[string]func() *http.Response{
			"https://api.github.com/repos/test/repo/pulls?direction=desc&per_page=100&sort=created&state=open": func() *http.Response {
				return &http.Response{
					StatusCode: http.StatusOK,
	...
	// 7. Verify the created IssueSandbox
	sandboxList := &unstructured.UnstructuredList{}
	sandboxList.SetGroupVersionKind(schema.GroupVersionKind{
		Group:   "custom.agents.x-k8s.io",
		Version: "v1alpha1",
	...
	email, found, err := unstructured.NestedString(sandbox.Object, "spec", "destination", "user", "email")
	g.Expect(err).NotTo(gomega.HaveOccurred())
	g.Expect(found).To(gomega.BeTrue())

	// The expected noreply email for ID 12345 and login test-user
	expectedEmail := "12345+test-user@users.noreply.github.com"

	g.Expect(email).To(gomega.Equal(expectedEmail))
}
